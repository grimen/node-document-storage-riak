...
var URL_AUTHORIZED = (process.env.RIAK_URL_AUTHORIZED = process.env.RIAK_URL_AUTHORIZED || undefined);
var URL_UNAUTHORIZED = (process.env.RIAK_URL_UNAUTHORIZED = process.env.RIAK_URL_UNAUTHORIZED || undefined);

console.log("\nRIAK_URL_AUTHORIZED = %s\nRIAK_URL_UNAUTHORIZED = %s", process.env.RIAK_URL_AUTHORIZED, process.env.RIAK_URL_UNAUTHORIZED);
  ...
  'Riak': {
    'new': {
      '()': function() {
        ...
        assert.equal ( storage2.url, 'http://localhost:8098/default-test' );
      ...
      '("url")': function() {
        ...
        var storage2 = new Storage('http://127.0.0.1:8098/custom');

        assert.equal ( storage2.url, 'http://127.0.0.1:8098/custom' );
        assert.typeOf ( storage2.options, 'object' );
        assert.deepEqual ( storage2.options.custom, undefined );
      },
      ...
      '("url", options)': function() {
        ...
        var storage2 = new Storage('http://127.0.0.1:8098/custom', {custom: {foo: 'bar'}});
    ...
    '.defaults': function() {
      ...
      assert.equal ( Storage.defaults.url, 'http://localhost:8098/default-test' );
      assert.typeOf ( Storage.defaults.options, 'object' );
    },

    '.url': function() {
      assert.typeOf ( Storage.url, 'string' );
      assert.equal ( Storage.url, 'http://localhost:8098/default-test' );
    },
    ...
    '.reset()': function() {
      ...
      Storage.url = "http://127.0.0.1:8098/custom";
      assert.equal ( Storage.url, "http://127.0.0.1:8098/custom" );
